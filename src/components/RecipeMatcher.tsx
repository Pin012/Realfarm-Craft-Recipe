...
export function RecipeMatcher({ recipes }: RecipeMatcherProps) {
  ...
  const getItemBackgroundClass = (name: string) => {
    if (name.includes('美味')) return 'bg-green-100 border-green-200';
    if (name.includes('優質')) return 'bg-blue-100 border-blue-200';
    if (name.includes('特製')) return 'bg-purple-100 border-purple-200';
